...
import logic_modules.HandValidation;
import logic_modules.NeighborshipValidation;
import logic_modules.ScoringModule;
...
public class Field {
	...
	public CheckProcessObject processMove(ArrayList<TileOnPositionEX> playtiles, ControlParameters control) {
		...
		int index = 0;
		ArrayList<TileOnPositionEX> copyPlaytiles = new ArrayList<>();
		for (TileOnPositionEX playtile : playtiles) {
			copyPlaytiles.add(playtile);
		}
// ######################################################################################################### FOR - LOOP NEIGHBORSHIP [START]
		while(index < copyPlaytiles.size()) {
			TileOnPositionEX playtile = copyPlaytiles.get(index);
			NeighborshipObject check_result = neighborshipValidation.find_neighborhood(playtile, valid_boardtiles);
			if(!check_result.isValid() && check_result.getFail_neighbor() != null && check_result.getFail_neighbor().getUniqueId() == playtile.getUniqueId()) {
				if((index + 1) < copyPlaytiles.size()) {
					index ++;
				}
			}
			else if(check_result.isValid()) {
				/**
				 * Easy logging block
				 */
