...
//##############################################################################################################################
	...
	public void ResumeGame(Message msg) {
		...
		Mainstarter.gameView.getPausePane().setVisible(false);
		Mainstarter.gameView.controlBar.resumeGame();
...
//##############################################################################################################################
	...
	public void Update(Message msg) {
		...
		GameController.update(u.getUpdates(), u.getNumberTilesInBag());
	}
	...
	public void LeavingPlayer(Message msg) {
		...
		GameController.informOfLeave(leavingClient);
		// TODO IMPLEMENT
	}
	...
	public void Winner(Message msg) {
		GsonBuilder builder = new GsonBuilder();
		Gson gson = builder.enableComplexMapKeySerialization().create();
		Type type = new TypeToken<Winner>(){}.getType();
		String jsonString = gson.toJson(msg, type);
		Winner winnerResponse = new GsonBuilder()
				.create().fromJson(jsonString, type);

		Client winner = winnerResponse.getClient();
		int winnerScore = winnerResponse.getScore();
		GameController.showWinner(winner, winnerScore);
	}
	...
	public void BagResponse(Message msg) {
		...
		GameController.updateBag(bagResponse.getBag());
	}
